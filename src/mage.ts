...
export function glucoseMAGE(
  ...
  const mean = validReadings.reduce((sum, v) => sum + v, 0) / validReadings.length;
  const variance = validReadings.reduce((sum, v) => sum + Math.pow(v - mean, 2), 0) / (validReadings.length - 1);
  ...
  const {
    shortWindow = Math.max(3, Math.min(5, Math.floor(validReadings.length / 8))),
    longWindow = Math.max(shortWindow + 2, Math.min(32, Math.floor(validReadings.length / 3))),
    direction = 'auto'
  } = options;
  ...
  try {
    ...
    const crossingPoints = _findMovingAverageCrossings(validReadings, shortWindow, longWindow);
 ...
 */
function _calculateMovingAverage(values: number[], windowSize: number): number[] {
  const result: number[] = [];
 ...
 */
function _findTurningPoints(readings: number[], crossings: number[]): TurningPoint[] {
 ...
 */
function _calculateExcursions(turningPoints: TurningPoint[], sd: number): Excursion[] {
 ...
 */
function _calculateMAGEFromExcursions(excursions: Excursion[], direction: 'auto' | 'ascending' | 'descending'): number {
  ...
  const filteredExcursions = excursions.filter(e => e.direction === targetDirection);
...
function _calculateSimpleMAGE(readings: number[], sd: number): number {
  ...
  for (let i = 0; i < turningPoints.length - 2; i++) {
    ...
    if ((tp1.type === 'peak' && tp2.type === 'nadir' && tp3.type === 'peak') ||
  ...
  return validAmplitudes.reduce((sum, amp) => sum + amp, 0) / validAmplitudes.length;
}
