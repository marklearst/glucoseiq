...
description: Turn normalized CGM readings into a complete React dashboard with live glucose, Time in Range, and an AGP profile.
---
...
By the end, you will have a current-glucose tile, Time-in-Range summary, AGP
profile, GMI, and data-sufficiency state without adding a charting library.
...
<Callout type="info" title="Start with enough history">
  ...
  the default sufficiency target is at least 14 days and 70% active time.
</Callout>

## 3. Build the dashboard

Create a client component and pass it the normalized readings. The option
objects live outside the component so their references remain stable across
renders.

```tsx title="GlucoseDashboard.tsx"
'use client'
...
Fetch readings in a server component or route, then hand the normalized array
to the dashboard:
