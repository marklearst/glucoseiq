...
A reading contains one glucose value, its unit, and a timestamp.
Source-specific details are optional metadata on that shape.
...
Keeping identity in the containing record limits the analytics input to
measurement data. The host still owns authorization and privacy controls.
