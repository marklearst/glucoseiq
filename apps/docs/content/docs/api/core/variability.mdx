...
**Remarks:** Returns NaN when `readings` is not an array, contains fewer than 2 values,
...
**Throws:** If either readings or percentiles is not an array (`INVALID_OPTION`)
...
**Remarks:** Returns the nearest-rank value for each percentile. Empty readings return an
...
**Remarks:** Returns NaN when `readings` is not an array or contains fewer than 2 values.
