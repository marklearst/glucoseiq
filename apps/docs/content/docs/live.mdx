...
This helper uses the exact timestamp string as the reading's ID. An incoming
reading replaces an earlier reading with the same timestamp string. Different
strings that describe the same time stay separate.
