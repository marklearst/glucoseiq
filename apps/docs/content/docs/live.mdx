...
newest value. Put the merge helper in `merge-readings.ts`:
...
The transport is intentionally yours. In `use-reading-feed.ts`, this polling
hook shows the boundary: the endpoint returns normalized readings, and the
