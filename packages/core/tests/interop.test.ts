...
describe('FHIR interop', () => {
  describe('buildFHIRCGMSummary', () => {
    it('builds a valid FHIR summary from EnhancedTIRResult', () => {
      ...
      // LOINC codes verified against the HL7 CGM IG v1.0.0 value set.
      const codes = summary.component.map((c) => c.code.coding[0].code)
      expect(codes).toContain('97510-2') // time in range, target
      expect(codes).toContain('104641-6') // TBR low
      expect(codes).toContain('104642-4') // TBR very low
      expect(codes).toContain('104640-8') // TAR high
      expect(codes).toContain('104639-0') // TAR very high
      // guard against the previously-transposed codes ever returning
      expect(codes).not.toContain('97507-0')
      expect(codes).not.toContain('97506-2')
    })

    it('includes optional meanGlucose component with the correct LOINC code', () => {
      const tir = calculateEnhancedTIR(sampleReadings)
      const summary = buildFHIRCGMSummary(tir, period, { meanGlucose: 130 })
      expect(summary.component.length).toBe(6)
      const meanComp = summary.component.find((c) => c.code.coding[0].code === '97507-8')
      expect(meanComp).toBeDefined()
      expect(meanComp!.valueQuantity.value).toBe(130)
    })

    it('includes optional GMI component', () => {
      const tir = calculateEnhancedTIR(sampleReadings)
      const summary = buildFHIRCGMSummary(tir, period, { gmi: 6.8 })
      const gmiComp = summary.component.find((c) => c.code.coding[0].code === '97506-0')
      expect(gmiComp).toBeDefined()
      expect(gmiComp!.valueQuantity.value).toBe(6.8)
    })

    it('includes optional CV component with the correct LOINC code', () => {
      const tir = calculateEnhancedTIR(sampleReadings)
      const summary = buildFHIRCGMSummary(tir, period, { cv: 22.5 })
      expect(summary.component.length).toBe(6)
      const cvComp = summary.component.find((c) => c.code.coding[0].code === '104638-2')
  ...
  describe('buildFHIRSensorReading', () => {
    ...
    it('builds mmol/L reading', () => {
      ...
      expect(result.code.coding[0].code).toBe('105272-9')
    })
  })
